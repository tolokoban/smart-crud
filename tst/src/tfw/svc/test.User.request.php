<?php
include 'test.User.request.security';

/**
 * Service test.User.request
 *
 * @param $inputs
 * * __id__
 * * __page__
...
function execService( $inputs ) {
    ...
    $fields = Array( 'id', 'login', 'password', 'name', 'comment', 'roles', 'creation' );
    $sqlFields = implode( ',', array_map( "surroundWithQuotes", $fields ) );
    $sql = "SELECT $sqlFields FROM " . $DB->table( 'User' );
    $sqlWhere = '';
    ...
    // Select count.
    $stm = $DB->query( "SELECT Count(*) FROM " . $DB->table( 'User' ) . $sqlWhere );
    $row = $stm->fetch();
