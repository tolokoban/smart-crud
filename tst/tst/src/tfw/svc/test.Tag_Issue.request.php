<?php
include 'test.Tag_Issue.request.security';

/**
 * Service test.Tag_Issue.request
 *
 * @param $inputs
 * * __id__
 * * __page__
...
function execService( $inputs ) {
    ...
    $sql = "SELECT id, $sqlFields FROM " . $DB->table( 'Tag_Issue' );
    $sqlWhere = '';
    ...
    // Select count.
    $stm = $DB->query( "SELECT Count(*) FROM " . $DB->table( 'Tag_Issue' ) . $sqlWhere );
    $row = $stm->fetch();
